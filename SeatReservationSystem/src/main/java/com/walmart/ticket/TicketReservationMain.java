...
public class TicketReservationMain {
	...
	public static Integer nbrOfRows ;
	public static Integer nbrOfSeatsPerRow  ;
	...
	public static Map<STATUS,List<SeatHold>> initializeSeats()
	{
		...
		{		
			...
			seatsMap.put(STATUS.HOLD, holdSeats);
			seatsMap.put(STATUS.RESERVED, reservedSeats);
		}
		else
		{
	...
	public static void main(String[] args) {
		...
		TicketServiceImpl t = new TicketServiceImpl(seatsMap,nbrOfRows, nbrOfSeatsPerRow);

		System.out.println("Total number of seats at the venue="+t.numSeatsAvailable());
		
		SeatHold sh = t.findAndHoldSeats(10,"abc@test.com");
		for(Seat s :sh.getSeatLoc())
		{
			System.out.println("The following seat numbers are on hold ="+s.getRow()+" "+s.getColumn());
			System.out.println("The seat Numbers are  ="+(s.getRow()*nbrOfSeatsPerRow+s.getColumn()));
			
		}
		System.out.println("The following seat numbers are on hold by "+sh.getReservedBy());
